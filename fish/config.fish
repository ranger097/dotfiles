...
function nii
sudo nano /etc/nixos/configuration.nix $argv
end


function wii
sudo nano ~/.config/wezterm/wezterm.lua $argv
end

function hii
sudo nano ~/.config/hypr/hyprland.conf $argv
end
...
function fish_prompt
    ...
    if test "$TRANSIENT" = "1"
        ...
        /run/current-system/sw/bin/starship prompt --terminal-width="$COLUMNS" --status=$STARSHIP_CMD_STATUS --pipestatus="$STARSHIP_CMD_PIPESTATUS" --keymap=$STARSHIP_KEYMAP --cmd-duration=$STARSHIP_DURATION --jobs=$STARSHIP_JOBS
    end
end
