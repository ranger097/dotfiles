...
alias wii="sudo nano /home/ranger/.config/wezterm/wezterm.lua"
alias bii="sudo nano ~/.bashrc"
alias nii="sudo nano /home/ranger/.config/niri/config.kdl"
alias ls="lsd"
...
_starship() {
    ...
    case "${cmd}" in
        ...
        starship__completions)
            opts="-h --help bash elvish fish nushell power-shell zsh"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
